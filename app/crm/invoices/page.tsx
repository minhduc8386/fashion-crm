...
export default function InvoicesPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <CrmNavbar title="🧾 Quản lý hoá đơn" />

      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold">Danh sách hoá đơn</h2>
            <p className="text-slate-400 text-sm mt-1">Tổng cộng: {invoices.length} hoá đơn</p>
          </div>
          <div className="flex items-center gap-3">
            {submitStatus === "success" && (
              <span className="text-green-400 text-sm bg-green-400/10 border border-green-400/20 px-3 py-1.5 rounded-lg">
            ...
            <button
              ...
              className="bg-purple-600 hover:bg-purple-500 text-white font-medium px-5 py-2.5 rounded-lg transition-colors"
            >
        ...
        {showForm && (
          <div className="bg-slate-900 border border-white/10 rounded-xl p-6 mb-6">
            <h3 className="text-white font-semibold mb-4">Tạo hoá đơn mới</h3>
            <form onSubmit={handleSubmit} id="form-create-invoice" className="space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500 transition-colors"
                >
                  ...
                  {items.map((item, i) => (
                    <div key={i} className="grid grid-cols-12 gap-2 items-center">
                      <input
                        ...
                        className="col-span-5 bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 text-sm transition-colors"
                      />
                      <input
                        id={`item-qty-${i}`} type="number" value={item.quantity} min={1}
                        onChange={(e) => updateItem(i, "quantity", parseInt(e.target.value) || 1)}
                        className="col-span-2 bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500 text-sm transition-colors"
                      />
                      <input
                        id={`item-price-${i}`} type="number" value={item.price} min={0}
                        onChange={(e) => updateItem(i, "price", parseFloat(e.target.value) || 0)}
                        placeholder="0"
                        className="col-span-4 bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 text-sm transition-colors"
                      />
                      <button type="button" onClick={() => items.length > 1 && setItems(items.filter((_, idx) => idx !== i))}
                        className="col-span-1 text-red-400 hover:text-red-300 text-center transition-colors disabled:opacity-30" disabled={items.length === 1}>
                        ✕
                      </button>
                    </div>
                  ))}
              ...
              <div className="flex justify-end">
                <div className="bg-slate-800 rounded-xl px-6 py-3 text-right">
                  <p className="text-slate-400 text-xs mb-1">Tổng tiền thanh toán</p>
                  <p className="text-purple-300 font-bold text-2xl">{formatCurrency(totalAmount)}</p>
                </div>
              </div>
              ...
              <div className="flex justify-end">
                <button id="btn-save-invoice" type="submit" disabled={submitStatus === "loading"}
                  className="bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-semibold px-8 py-3 rounded-lg transition-colors">
        ...
        <div className="bg-slate-900 border border-white/10 rounded-xl overflow-hidden">
          {loading ? (
            <div className="space-y-3 p-6">
              {[...Array(5)].map((_, i) => <div key={i} className="h-10 bg-slate-800 rounded-lg animate-pulse" />)}
            </div>
          ) : invoices.length === 0 ? (
            <div className="text-center py-16 text-slate-400">Chưa có hoá đơn nào.</div>
          ) : (
              ...
              <tbody>
                {invoices.map((inv, i) => (
                  <tr key={inv._id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white font-medium">{inv.customer_name}</td>
                    <td className="px-4 py-3 text-slate-300 hidden sm:table-cell">
                      {inv.items?.map((it) => `${it.product_name} x${it.quantity}`).join(", ") || "–"}
                    </td>
                    <td className="px-4 py-3 text-purple-300 font-semibold text-right">{formatCurrency(inv.total_amount)}</td>
                    <td className="px-4 py-3 text-slate-400 hidden md:table-cell">{formatDate(inv.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
